    ...
    for line in lines:
        ...
        if trip_type:
            if "Optimal fuel types" in line:
                ...
                if "penalties" not in results[trip_type]:
                    results[trip_type]["penalties"] = {}
                results[trip_type]["penalties"].setdefault(penalty_key, 0)
                results[trip_type]["penalties"][penalty_key] += penalty_value
            elif "OPS Penalty" in line:
                ops_penalty = float(line.split(':')[1].strip().split('€')[0])
                if "penalties" not in results[trip_type]:
                    results[trip_type]["penalties"] = {}
                results[trip_type]["penalties"].setdefault("OPS", 0)
                results[trip_type]["penalties"]["OPS"] += ops_penalty
...
for year in range(2025, 2051, 5):
    for CO2_price in range(90, 191, 20):
        ...
        for base_scenario in base_scenarios:
            if scenario_count > 4:
                scenario_count = 1
            scenario = {'year': year, 'CO2_price': CO2_price, **base_scenario}
